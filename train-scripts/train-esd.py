...
@torch.no_grad()
def sample_model(model, sampler, c, h, w, ddim_steps, scale, ddim_eta, start_code=None, n_samples=1, t_start=-1,
                 log_every_t=None, till_T=None, verbose=True):
    """Sample the model"""
    uc = None
    if scale != 1.0:
...
def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path,
              diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50):
    '''
        ...
        if train_method == 'xlayer':
            if 'attn2' in name:
                if 'output_blocks.6.' in name or 'output_blocks.8.' in name:
                    ...
                    param_keys.append(name)
                    parameters.append(param)
        if train_method == 'outxattn':
            if 'attn2' in name:
                if 'output_blocks.' in name:
                    print(name)
                    param_keys.append(name)
                    parameters.append(param)
        if train_method == 'outsattn':
            if 'attn1' in name:
                if 'output_blocks.' in name:
                    print(name)
                    param_keys.append(name)
                    parameters.append(param)
        if train_method == 'inxattn':
            if 'attn2' in name:
                if 'input_blocks.' in name:
                    print(name)
                    param_keys.append(name)
                    parameters.append(param)
        if train_method == 'midxattn':
            if 'attn2' in name:
                if 'middle_block.' in name:
                    print(name)
                    param_keys.append(name)
                    parameters.append(param)
        if train_method == 'inmidsattn':
            if 'attn1' in name:
    ...
    losses = []
    opt = bitsandbytes.optim.AdamW8bit(parameters, lr=lr)
    ...
    for i in pbar:
        ...
        loss = criteria(e_n.to(devices[0]), e_0.to(devices[0]) - (negative_guidance * (e_p.to(devices[0]) - e_0.to(
            devices[0]))))  # loss = criteria(e_n, e_0) works the best try 5000 epochs
        loss.backward()
        ...
        if (i + 1) % 50 == 0 and i + 1 != iterations and i + 1 >= 50:
            save_model(model, name, i - 1, save_compvis=True, save_diffusers=False, save_only_params=True,
    ...
    save_model(model, name, None, save_compvis=True, save_diffusers=True, compvis_config_file=config_path,
               diffusers_config_file=diffusers_config_path, param_keys=param_keys)
    save_history(losses, name, word_print)


def save_model(model, name, num, compvis_config_file=None, diffusers_config_file=None, device='cpu', save_compvis=True,
               save_diffusers=True, save_only_params=False, param_keys=None):
    # SAVE MODEL
    ...
    if save_compvis:
        output_dict = model.state_dict()
        if save_only_params and param_keys is not None:
            # filter out unnecessary keys
            output_dict = {'model.diffusion_model.' + k: output_dict['model.diffusion_model.' + k] for k in param_keys}
...
if __name__ == '__main__':
    ...
    train_esd(prompt=prompt, train_method=train_method, start_guidance=start_guidance,
              negative_guidance=negative_guidance, iterations=iterations, lr=lr, config_path=config_path,
              ckpt_path=ckpt_path, diffusers_config_path=diffusers_config_path, devices=devices, seperator=seperator,
